...
% Simulate erosion of imaginary-coded memories over time
...
x_bar = randn;
% connectivity matrix
W = zeros(N); % start with all zeros
% W = randn(N)/sqrt(N); % start with non-zeros
...
[~,I] = sort((imag(Dseq(:,26))),'descend');

%% plotting
taxis = [1:1:Nsteps]*CalcEvery*dt;
for j=1:1:N 
    plot(taxis,imag(Dseq(j,:)),'linewidth',2,'color',[1 1 1]*.8); 
    hold on;
end
plot(taxis,imag(Dseq(I(1),:)),'linewidth',2,'color',[0 0.4470 0.7410]);  
hold on;
